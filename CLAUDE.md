...
- **oEmbed enrichment runs through the SAME `preview.Fetcher` client — never a second HTTP stack.** `internal/preview/oembed.go:fetchOEmbed` reuses `f.client` and its SSRF-guarded transport. **Scheme guard at the edge is critical**: an `OEmbedURL` captured from `<link rel="alternate" type="application/json+oembed">` is attacker-controlled HTML; Go's default transport happily reads `file:///etc/passwd` because the IP-level dialer never fires for non-`http(s)` schemes. `fetchOEmbed` enforces `u.Scheme ∈ {http, https}` BEFORE `http.NewRequestWithContext`. Same posture as the metadata handler. Discovery URL is resolved against the page's `finalURL` via `resolveRelatives` so path-relative `href`s (WordPress, SoundCloud, Flickr) reach the right host. Sub-deadline of 5 s on each oEmbed leg bounds wall-clock independently of the caller's ctx. `knownOEmbedProviders` (YouTube hosts + Vimeo) shortcuts straight to oEmbed when the page host matches; everyone else gets HTML fetch + opportunistic discovery enrichment. Merge contract: HTML always wins what it has — oEmbed only fills empty fields.
...
- **Backup export streams**, **restore streams to temp file.** `backup.Service.Export(ctx, w, onCountsReady)` reads snapshot + `ListObjects` under REPEATABLE READ, fires the callback (handler flushes `X-Foldex-Backup-*` headers there), then streams the ZIP straight to `w`. `readZipFromRequest` writes upload via `io.Copy` to `os.CreateTemp` and opens with `zip.NewReader(*os.File, n)`. Cleanup on success and failure. Do not reintroduce `bytes.Buffer` in either path — 2 GiB restore would peg the heap.
