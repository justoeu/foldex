...
| Tiptap | `^3.27` (`@tiptap/react` + `@tiptap/starter-kit` + `@tiptap/extension-image` + `@tiptap/extension-placeholder`) | Rich-text editor for notes. `@tiptap/extension-link` is NOT a separate dep — StarterKit bundles it; configure via `StarterKit.configure({ link: {...} })`. |
