...
- **Internal IDs never appear in the URL.** Folder navigation, link IDs, etc. live in component state only — no `?folder=N`, no `/folder/:id` route. The address bar shows only the bare app URL. Same rule for tooltips: `/go/:id` is an implementation detail, the UI label is just "Acessar".
