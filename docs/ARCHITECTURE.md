...
|        | GET    | `/api/links/url-metadata?url=…`       | Pré-fetch síncrono usado pelo `LinkDialog` para auto-preencher Título/Descrição assim que o usuário cola/digita uma URL. Reusa `preview.NewFetcher` (mesmo SSRF gate, mesma posture); rejeita scheme não-http(s) → 400 `invalid_scheme`, URL > 2 KiB → 400 `invalid_url`. Falhas de fetch (DNS, SSRF, 4xx, timeout) colapsam em 502 `fetch_failed` sem vazar a mensagem interna. Debounce 500 ms no front; o backend é fire-on-demand sem cache. |
