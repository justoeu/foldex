...
|        | GET    | `/api/links/by-url?url=…`             | Probe de unicidade owner-scoped para o diálogo de novo link. 200 com a linha do chamador; 404 se não existe **ou** é de outro tenant (INV-050 / INV-054). URL vazia → 400 `invalid_input`; > 2 KiB → 400 `invalid_url`. Não atravessa pasta trancada: é lookup da própria linha, não listagem de conteúdo. |
|        | POST   | `/api/links/url-metadata`             | Pré-fetch síncrono usado pelo `LinkDialog` para auto-preencher Título/Descrição assim que o usuário cola/digita uma URL. Body `{url}` (query `url` ainda aceito). POST para CSRF, `writeGate` (`content.write`) e a cota mutante — viewer/token sem write → 403 sem dial. Slot in-flight (2 global, 1/user) recusa extras com 429 `metadata_busy` antes do fetch. Reusa `preview.NewFetcher` (mesmo SSRF gate, mesma posture); rejeita scheme não-http(s) → 400 `invalid_scheme`, URL > 2 KiB → 400 `invalid_url`. HTTP/oEmbed primeiro (teto 5 s); se o origin 401/403/429, timeout, ou título vazio/interstitial, o mesmo pool Chromium do screenshot lê o DOM renderizado (INV-083/084/085, teto 10 s, só URL pública). DNS/TLS/5xx/SSRF nunca disparam o browser. Se os dois passes falham: **200 com campos vazios** (não 502) — bot wall de terceiro não é falta da API. Debounce 500 ms + cache módulo-level com TTL 5 min no front; backend é fire-on-demand sem cache próprio. **YouTube/Vimeo bypassam HTML via oEmbed** (`preview.Fetcher.Fetch` curto-circuita pro endpoint oEmbed quando host bate `knownOEmbedProviders`, porque YouTube serve HTML degraded a fingerprints de container — UA/headers/cookies não ajudam). Discovery genérico via `<link rel="alternate" type="application/json+oembed">` enriquece outros sites quando o HTML carrega mas falta título/imagem. |
...
- SSRF guard: a policy compartilhada em `internal/pkg/netpolicy` bloqueia metadata/credenciais cloud e RFC6598 sempre, sem opt-out. Fetch HTML bloqueia por padrão os demais ranges special-purpose da IANA (loopback, RFC1918, benchmarking/documentação/reservado, link-local e IPv6 ULA). `PREVIEW_STRICT_SSRF=0` permite intranet explicitamente para usuários confiáveis; vazio, ausente ou inválido mantém strict.
...
POSTGRES_PASSWORD=        # gerada por make env; sem fallback no Compose
POSTGRES_DB=foldex
POSTGRES_PORT=5432
...
PREVIEW_STRICT_SSRF=1       # padrão estrito; 0 permite intranet explicitamente
CORS_ORIGINS=*
...
Fetcher visita URLs fornecidas por usuários com permissão de escrita. **Ranges de metadata/credenciais cloud e RFC6598 são sempre bloqueados**, sem opt-out. Os demais ranges special-purpose da IANA também são bloqueados por padrão, antes e depois do dial. O operador pode definir `PREVIEW_STRICT_SSRF=0` para bookmarks de intranet de usuários confiáveis; essa opção permite alcançar serviços internos do host. Screenshot e Web Push não aceitam esse opt-out.
...
### ADR-12 — SSRF guard estrito por default (emendado)
**Emenda de segurança:** o padrão agora é strict, inclusive quando `PREVIEW_STRICT_SSRF` está vazio ou inválido. Intranet (Jira, Grid, Confluence e dashboards internos) continua suportada com `PREVIEW_STRICT_SSRF=0` explícito; `false` e `no` também são aceitos, sem distinção de caixa e com espaços externos removidos. O operador decide conceder aos usuários acesso à rede do servidor. Metadata/credenciais cloud e RFC6598 continuam bloqueados em todos os modos.
...
**Segunda passada (18 alertas do backup/abuso, 2026-09-01).** Todos ruído: gosec G104 em `Close`/`Remove` de spool (`_ =` no defer); G204 em `pg_dump`/`execCommand` (binário literal ou seam de teste); G304 no drill (`//nolint:gosec` não é a sintaxe do gosec — passou a `#nosec G304`); G404 + Semgrep `math/rand` no jitter de catch-up (atraso de agenda, não credencial); CodeQL `go/log-injection` no `job` da agenda (URL/JSON) e no `err` do `abuse policy set`. O `job` agora é internado por `CanonicalJob` (constante do pacote, nunca a string do request). O `err` do Set depois de `ValidateForWrite` é falha de banco/encode — o log ficou estático de propósito, senão o dataflow re-taintava o sink com o body.
...
**Login indistinguível.** E-mail inexistente, senha errada, conta desabilitada **e lockout do balde por conta** devolvem `401 invalid_credentials` byte-idêntico. bcrypt **sempre** roda (contra um hash dummy no miss — pular é o oráculo clássico de ~80 ms), o bucket de rate limit por e-mail incrementa **também** para e-mails inexistentes (não incrementar é por si só um oráculo), e há um **piso** de 250 ms — piso e não jitter, porque jitter só adiciona variância que o atacante remove com média amostral. O 429 fica só no balde de origem (spray); um 429 no balde por conta ligava username à caixa.
