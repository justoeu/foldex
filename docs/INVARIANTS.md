...
<a id="inv-041"></a>
### INV-041 — Login is byte-identical for unknown e-mail, wrong password, disabled account and per-account lockout.

*Guards:* `TestLogin_FailuresAreByteIdentical`, `TestLogin_AccountLockoutDoesNotDistinguishAnAliasFromAStranger`

Five mechanisms, all required: bcrypt **always runs** (against a dummy hash on a miss — skipping it is the classic ~80 ms oracle); one `401 invalid_credentials` body for unknown, wrong, disabled **and a locked account bucket**; the per-e-mail rate bucket increments **for unknown addresses too** (not incrementing teaches the attacker which addresses are lockable, hence which exist); a **250 ms duration floor**, not jitter — jitter only adds variance that averaging removes; and the account lockout itself **must not grow a 429**. The per-account bucket is keyed on the resolved account so username and mailbox share one depth budget (INV-184); a 429 on the sixth request of one name and a 401 on the other is an unauthenticated username-to-mailbox oracle — the question INV-013 keeps behind a session. Origin-bucket 429 stays 429: its members are the submitted identifiers, so it cannot tell two names of one account from two names of none. `internal/pkg/attemptlimit`'s reserve-then-commit API (`Begin`/`Release`/`CommitFail`/`CommitSuccess`) is what makes the cap hold under concurrency: a plain check-then-act lets N parallel guesses all read the same pre-cap count while bcrypt runs.
...
**Corolário (ADR-49): a identidade privada entra por arquivo 0600 OU inline, e a inline é lida uma vez e apagada do ambiente.** `BACKUP_AGE_IDENTITY` existe para a plataforma que não consegue produzir um mount 0600; as duas fontes ao mesmo tempo recusam o boot, e a regra de modo do arquivo não afrouxou. `configFromEnv` faz `os.Unsetenv` no mesmo instante em que lê, incondicionalmente — `pg_dump` herda `os.Environ()`, e a chave que abre todos os backups não pode chegar a um filho. O ambiente mínimo dos filhos do `pg_restore` já a excluía. Guards: `TestLoad_ScrubsTheInlineIdentityFromTheEnvironment`, `TestLoad_RefusesBothIdentitySources`, `TestExecCommand_ChildrenNeverSeeTheAgentsSecrets`, `TestNew_InlineIdentityOpensTheDownloadBridge`, `TestAgent_HeartbeatCarriesNoCredential`.
...
- **Só métodos mutantes, com uma exceção declarada.** Ler é um `SELECT` indexado; contabilizá-lo gastaria o orçamento de quem navega a própria biblioteca sem tocar na amplificação de escrita que a cota existe para conter. `GET /api/export` é a exceção: materializa a biblioteca inteira (o mesmo custo de um import, no sentido inverso) e por isso entra em `expensiveRoutes`. O middleware deixa passar um GET que **não** é caro; um GET que é fica no balde horário. Continua GET — e não POST — para o `<a href>` nativo da página de importação continuar sendo um download, não um fetch com CSRF.
...
- **O lockout por CONTA responde 401, não 429.** O balde continua chaveado na conta resolvida (`loginBucketKey`), então cinco senhas erradas em `alice` trancam o mesmo balde que `alice@x.com` — profundidade compartilhada, que é o ponto. A sexta requisição, porém, devolve o mesmo `401 invalid_credentials` de um miss, sem `Retry-After`: um 429 naquele ponto era a pergunta que o INV-013 mantém autenticada, lida de fora. O balde de ORIGEM continua 429 porque seu membro não é resolvido. Guard: `TestLogin_AccountLockoutDoesNotDistinguishAnAliasFromAStranger`.
