...
<a id="inv-087"></a>
### INV-087 — `GET /api/links/url-metadata` reuses the preview `Fetcher` — same SSRF posture, no duplicate HTTP client.
...
*Guards:* `AuthGate.transition.test.tsx` — `asks for the code screen while the sign-in form is still on screen`, `keeps the sign-in screen on the glass while the code screen loads`, `swaps in the code screen once it arrives`

O `AuthGate` carrega seis telas por `lazy()`, e o fallback do `Suspense` é o `boot` — `.fx-auth fx-auth-boot`, uma sobreposição de viewport inteira. Para quatro delas isso é honesto: `ResetScreen`, `ForgotScreen`, `VerifyEmailScreen` e `ConvertScreen` são alcançadas no MOUNT, a partir de um token na URL, quando ainda não há nada pintado e o spinner de chunk é indistinguível do probe do `/me`.

As duas telas de desafio são de outra natureza. `TwoFactorScreen` e `EnrollTotpScreen` são alcançadas por uma TRANSIÇÃO a partir do formulário de login já pintado — dois estados de UM fluxo. Como atualização urgente, o React commita o fallback, e o operador vê a página inteira piscar entre "Entrar" e o pedido do código: o card não trocou, a tela apagou. Duas regras, e as duas são necessárias:

1. **A troca de sessão é uma `startTransition`.** É o que faz o React SEGURAR a tela anterior enquanto a nova suspende, em vez de commitar o fallback. Sem isso nenhum prefetch resolve o caso ruim — um chunk lento continua apagando a tela. Fica em `applySession`, o ponto único por onde login, convite, reset e o próprio 2FA passam; nenhuma outra transição de sessão suspende (as árvores de login e do app são eager), então para todas as outras a atualização é a mesma de sempre.
2. **Os dois chunks de desafio são aquecidos enquanto o formulário está na tela.** É o que torna a espera imperceptível em vez de apenas invisível. O segundo fator é o DEFAULT (`AUTH_REQUIRE_2FA_FOR_ADMINS`), então quem está olhando o formulário está a um submit de uma dessas telas; pagar o chunk ali é o que colocou o spinner no meio do fluxo. Quem já está autenticado nunca chega ao efeito — que é exatamente a visita para a qual o split foi feito, e por isso ele continua valendo.
