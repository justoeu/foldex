...
| **New link**               | `⌥N` ou botão `+` abre dialog. URL → cola → ~500 ms depois, backend devolve título e descrição da página (via `GET /api/links/url-metadata`) e o dialog auto-preenche os campos que ainda estão vazios. Imagem segue assíncrona via preview worker. |
