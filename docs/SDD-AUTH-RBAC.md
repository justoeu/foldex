...
| `GET` | `/api/auth/username-available?u=` | sessão | `{available, reason?}` com `reason ∈ {shape, reserved, taken, empty}` (o e-mail acrescenta `pending`, que anda junto de `available: true`). Sempre 200 (429 no estouro do teto). **É um oráculo de enumeração por construção** — responde sobre o identificador de OUTRA conta e não pede senha. O que o torna aceitável: só sessão, só USERNAME (nunca e-mail — um endereço é também uma caixa e existe fora desta instância), e teto de 60 por 5 min por usuário. O orçamento é cobrado ANTES da consulta e também para formas recusadas, senão basta acrescentar um caractere inválido para sondar de graça. Probe vazia é gratuita: não consulta nada. O próprio username do chamador conta como disponível. |
| `GET` | `/api/admin/users/email-available?email=` | sessão + admin (`roles.assign`) | `{available, reason?}`. **A contrapartida de e-mail existe SÓ aqui**, e a colocação é o argumento inteiro: depois do `RequireAdmin` o chamador já lista todas as contas com endereço, então não revela nada que ele não leia direto — e por isso não tem teto. Uma troca de e-mail PENDENTE responde `available: true` com `reason: "pending"`, nunca `taken`: o `AdminCreateUser` conflita só em `app_user`, então recusar apagaria um Create que o servidor aceitaria, sem saída visível — e a linha pendente ainda pode expirar ou morrer com a época de credencial. É um aviso, não uma recusa. Nunca espelhar numa rota que uma sessão comum alcance. |
