...
func TestRepositoryHTTPErrorMapping(t *testing.T) {
	tests := []struct {
		...
		{"URL taken", fmt.Errorf("create: %w", ErrURLTaken), http.StatusConflict, "url_taken", "url already in use"},
		{"slug taken", fmt.Errorf("create: %w", ErrSlugTaken), http.StatusConflict, "slug_taken", "slug already in use"},
		{"stale write", fmt.Errorf("update: %w", ErrStaleWrite), http.StatusConflict, "conflict", "link was modified; refetch and retry"},
