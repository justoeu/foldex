...
func restoreDuplicate(ctx context.Context, tx pgx.Tx, uid authctx.UserID, snap *Snapshot) (Counts, []string, idMapping, error) {
	...
	for _, t := range snap.Tags {
		name, err := uniqueTagName(ctx, tx, uid, t.Name)
		if err != nil {
			return inserted, warnings, m, err
		}
		var newID int64
		if err := tx.QueryRow(ctx,
			`INSERT INTO tag (user_id, name, color, icon, created_at) VALUES ($1,$2,$3,$4,$5) RETURNING id`,
			int64(uid), name, t.Color, t.Icon, t.CreatedAt).Scan(&newID); err != nil {
