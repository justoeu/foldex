...
func loadRestoreLedger(ctx context.Context, db restoreLedgerQuerier, uid authctx.UserID, digest [sha256.Size]byte, mode ConflictMode) (restoreLedger, bool, error) {
	...
	if err := decodeRestoreLedgerJSON(&ledger, insertedJSON, skippedJSON, warningsJSON, filesJSON); err != nil {
		return ledger, false, err
	}
	if ledger.complete {
		return ledger, true, nil
	}
	if err := loadRestoreEntityMappings(ctx, db, uid, digest, mode, &ledger); err != nil {
		return ledger, false, err
	}
	if err := loadRestoreFileMappings(ctx, db, uid, digest, mode, &ledger); err != nil {
		return ledger, false, err
	}
	return ledger, true, nil
}

func decodeRestoreLedgerJSON(ledger *restoreLedger, insertedJSON, skippedJSON, warningsJSON, filesJSON []byte) error {
	if err := json.Unmarshal(insertedJSON, &ledger.inserted); err != nil {
		return fmt.Errorf("decode restore inserted counts: %w", err)
	}
	if err := json.Unmarshal(skippedJSON, &ledger.skipped); err != nil {
		return fmt.Errorf("decode restore skipped counts: %w", err)
	}
	if err := json.Unmarshal(warningsJSON, &ledger.warnings); err != nil {
		return fmt.Errorf("decode restore warnings: %w", err)
	}
	if len(filesJSON) == 0 {
		return nil
	}
	if err := json.Unmarshal(filesJSON, &ledger.files); err != nil {
		return fmt.Errorf("decode restore file report: %w", err)
	}
	return nil
}

func loadRestoreEntityMappings(ctx context.Context, db restoreLedgerQuerier, uid authctx.UserID, digest [sha256.Size]byte, mode ConflictMode, ledger *restoreLedger) error {
	...
	if err != nil {
		return fmt.Errorf("load restore entity mappings: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var kind string
		...
		if err := rows.Scan(&kind, &sourceID, &targetID); err != nil {
			return fmt.Errorf("scan restore entity mapping: %w", err)
		}
		if err := applyRestoreEntityMapping(ledger, kind, sourceID, targetID); err != nil {
			return err
		}
	}
...
func applyRestoreEntityMapping(ledger *restoreLedger, kind string, sourceID, targetID int64) error {
	switch kind {
	case "tag":
	...
	default:
		return fmt.Errorf("load restore entity mapping: unknown kind %q", kind)
	}
	return nil
}

func loadRestoreFileMappings(ctx context.Context, db restoreLedgerQuerier, uid authctx.UserID, digest [sha256.Size]byte, mode ConflictMode, ledger *restoreLedger) error {
	fileRows, err := db.Query(ctx, `
		...
		int64(uid), digest[:], string(mode))
	if err != nil {
		return fmt.Errorf("load restore file mappings: %w", err)
	}
	defer fileRows.Close()
	for fileRows.Next() {
		var sourceKey, targetKey string
		if err := fileRows.Scan(&sourceKey, &targetKey); err != nil {
			return fmt.Errorf("scan restore file mapping: %w", err)
		}
