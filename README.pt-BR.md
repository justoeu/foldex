...
Backup nunca restaurado é esperança, não backup — por isso o agente também roda um **restore drill** automático (`BACKUP_DRILL_AT`, ex.: `04:30 sun`, desligado por default): ele baixa o dump mais recente direto do bucket, confere o SHA-256 registrado, decifra com a identidade privada (`BACKUP_AGE_IDENTITY_FILE`, montada read-only, modo 0600), restaura num cluster Postgres descartável dentro do próprio container do agente e compara as contagens por tabela e a versão do schema com o que o dump registrou na origem. Qualquer divergência vira falha normalizada (`drill_digest_mismatch`, `drill_counts_mismatch`, …) em `backup_run` e nas métricas.

O agente também **espelha o bucket RustFS** (screenshots, mídia de notas) no mesmo bucket externo sob `backups/rustfs/`, incrementalmente a cada 6 horas por default (`BACKUP_MIRROR_INTERVAL_MIN`, `0` desliga): cada objeto sai **cifrado com age** (`<chave>.age`, mesmos recipients dos dumps), cada passada difere por presença e watermark de last-modified (mais tamanho no modo plaintext), e **deleções nunca se propagam** — um wipe na origem, bug ou ransomware, não apaga a cópia de backup. Um restore per-user em andamento faz o espelho esperar em vez de enviar um bucket escrito pela metade.
...
Cada execução fica registrada na tabela `backup_run`, e o agente serve métricas Prometheus (`/metrics`, mesmo `METRICS_TOKEN` do backend) com alert rules prontas para Grafana/Prometheus em [`deploy/observability/`](deploy/observability/) — incluindo uma regra `absent()` que pega o agente que nunca subiu. Configuração pelo bloco `BACKUP_*` do `.env.example`.
