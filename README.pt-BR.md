...
Cada execução fica registrada na tabela `backup_run`, e o agente serve métricas Prometheus (`/metrics`, mesmo `METRICS_TOKEN` do backend) com alert rules prontas para Grafana/Prometheus em [`deploy/observability/`](deploy/observability/) — incluindo uma regra `absent()` que pega o agente que nunca subiu e uma regra de staleness para os drills. Configuração pelo bloco `BACKUP_*` do `.env.example`. A UI de status admin chega na próxima release desta funcionalidade (ver faseamento no SDD).
