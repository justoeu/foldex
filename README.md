...
Foldex is a personal "smart bookmarks bar" — it stores links organized by **nestable folders + M:N tags** (a new folder or tag opens on a suggested colour nothing else is using), shows **what you actually click** (telemetry via `/go/{slug}`), captures every URL visually (OG image / favicon / screenshot fallback, and a preview whose stored file goes missing regenerates itself), lets you jot down **rich-text notes** (Tiptap editor with inline images) that live in the same grid/search/tags/folders as links, **watches the pages you care about** (RSS/Atom feed fingerprint with content-hash fallback) and pings you via Web Push when they change, and runs **entirely on your own machine** (Postgres + RustFS + Go + React in containers).
...
> **Reuse a Postgres you already run on your host.** Set `POSTGRES_HOST=host.docker.internal` — or `localhost`, which works too — in `.env` (and matching `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB`), skip `make db-up`, and run `make apps-up` directly. `make migrate-up` reaches a host Postgres on either setting: the migrate CLI runs in its own container, where `localhost` would be that container, so the target rewrites the host to `host.docker.internal` for it. Any other hostname is passed through untouched, so a remote Postgres is dialled as written.

## Stack layout

Three compose projects on the shared `foldex` Docker network. Postgres lives in `docker-compose.db.yml`; the RustFS object store lives in `docker-compose.services.yml`; `docker-compose.yml` is the APP only — backend, web, and the `mailer` worker under `COMPOSE_PROFILES=amqp` — reaching the others by the names `db` and `rustfs`.

The app file carries no service you might already run elsewhere, and that is structural rather than tidiness: compose interpolates **every** service before starting any of them, and profiles do not exempt what they exclude, so a `${VAR:?...}` on a store you host externally refuses to start the backend and the web too. `make up` brings Postgres and RustFS along only when `POSTGRES_HOST` / `RUSTFS_ENDPOINT` point at the bundled ones. Useful targets (`make help`):

| Target | What |
|---|---|
| `make db-up` / `db-down` / `db-nuke` | manage Postgres only |
| `make storage-up` / `storage-down` / `storage-logs` | manage the RustFS object store only |
| `make apps-up` / `down` | manage backend + web (+ `mailer` under `COMPOSE_PROFILES=amqp`) |
| `make up` / `stop-all` | full stack — brings Postgres and RustFS along only when `POSTGRES_HOST`/`RUSTFS_ENDPOINT` point at the bundled ones |
...
A backup that was never restored is hope, not a backup — so the agent also runs an automatic **restore drill** (`BACKUP_DRILL_AT`, e.g. `04:30 sun`, off by default): it downloads the newest dump straight from the bucket, verifies the recorded SHA-256, decrypts it with the private identity (`BACKUP_AGE_IDENTITY_FILE`, mounted read-only, mode 0600), restores it into a disposable Postgres cluster inside the agent's own container and compares per-table row counts plus the schema version against what the dump recorded at the source. Any divergence lands as a normalized failure (`drill_digest_mismatch`, `drill_counts_mismatch`, …) in `backup_run` and in the metrics.

The agent also **mirrors the RustFS bucket** (screenshots, note media) into the same external bucket under `backups/rustfs/`, incrementally every 6 hours by default (`BACKUP_MIRROR_INTERVAL_MIN`, `0` turns it off): each object ships **age-encrypted** (`<key>.age`, same recipients as the dumps), each pass diffs by presence and a last-modified watermark (plus size in plaintext mode), and **deletions never propagate** — a wipe at the origin, bug or ransomware, cannot erase the backup copy. A per-user restore in progress makes the mirror wait instead of shipping a half-written bucket.
...
Every run is recorded in the `backup_run` table, and the agent serves Prometheus metrics (`/metrics`, same `METRICS_TOKEN` as the backend) with ready-made Grafana/Prometheus alert rules in [`deploy/observability/`](deploy/observability/) — including an `absent()` rule that catches the agent that never started. Configure via the `BACKUP_*` block in `.env.example`.
...
```bash
# In .env, next to each other — they must agree:
#   MAIL_TRANSPORT=amqp
#   COMPOSE_PROFILES=amqp
docker compose up -d
```

`COMPOSE_PROFILES` is not a convenience. The `mailer` service carries
`profiles: ["amqp"]`, so bringing the stack up without it starts a backend that
publishes and no worker that consumes — and that failure is silent in both
directions: the publish is confirmed, the outbox row settles as `published`, and
the first sign of it is a user reporting an e-mail that never arrived. The
backend now logs a warning whenever it publishes into a queue with no consumer,
and the worker logs one line per message it sends.

| Var | Default | Meaning |
|---|---|---|
| `MAIL_TRANSPORT` | `inproc` | `inproc` \| `amqp`. An unknown value refuses to boot |
| `COMPOSE_PROFILES` | — | Set to `amqp` alongside `MAIL_TRANSPORT=amqp` so `docker compose up -d` starts the worker |
| `AMQP_URL` | — | Required for `amqp`. `amqp://` to a **remote** host is refused; use `amqps://` |
| _(private TLS)_ | — | Configured IN THE URL, not by a variable: `?cacertfile=`, `?certfile=`/`?keyfile=` (mTLS) and `?server_name_indication=`. Under Docker, drop the PEM in `./certs` (mounted at `/etc/foldex/certs`) and use the container path. |
| `AMQP_ALLOW_PLAINTEXT` | `0` | Allows plaintext `amqp://` to a non-loopback broker. Verified against the network at boot **and at dial**, so a hostname resolving outside RFC1918/CGNAT/loopback/link-local is refused before the credential is sent. Recipient address and template name travel in clear; the message body stays sealed. |
| `AMQP_EXCHANGE` | `foldex.mail` | Rename only to share one broker between instances |
| `AMQP_QUEUE` | `foldex.mail.send` | |
| `AMQP_ROUTING_KEY` | `send` | Binds the queue to the exchange |
